...
namespace ArcGitSourceControlConstants
{
	/** The maximum number of files we submit in a single Git command */
	const int32 MaxFilesPerBatch = 50;
}

FArcGitScopedTempFile::FArcGitScopedTempFile(const FText& InText)
{
...
FArcGitScopedTempFile::~FArcGitScopedTempFile()
{
...
const FString& FArcGitScopedTempFile::GetFilename() const
{
	return Filename;
}


namespace ArcGitSourceControlUtils
{
...
bool CheckGitAvailability(const FString& InPathToGitBinary, FArcGitVersion *OutVersion)
{
#if PLATFORM_WINDOWS
...
void ParseGitVersion(const FString& InVersionString, FArcGitVersion *OutVersion)
{
...
void FindGitCapabilities(const FString& InPathToGitBinary, FArcGitVersion *OutVersion)
{
	FString InfoMessages;
	FString ErrorMessages;
...
void FindGitLfsCapabilities(const FString& InPathToGitBinary, FArcGitVersion *OutVersion)
{
	FString InfoMessages;
	FString ErrorMessages;
...
bool RunCommand(const FString& InCommand, const FString& InPathToGitBinary, const FString& InRepositoryRoot, const TArray<FString>& InParameters, const TArray<FString>& InFiles, TArray<FString>& OutResults, TArray<FString>& OutErrorMessages)
{
	...
	if(InFiles.Num() > ArcGitSourceControlConstants::MaxFilesPerBatch)
	{
		...
		while(FileCount < InFiles.Num())
		{
			...
			for(int32 FileIndex = 0; FileCount < InFiles.Num() && FileIndex < ArcGitSourceControlConstants::MaxFilesPerBatch; FileIndex++, FileCount++)
			{
...
bool RunCommit(const FString& InPathToGitBinary, const FString& InRepositoryRoot, const TArray<FString>& InParameters, const TArray<FString>& InFiles, TArray<FString>& OutResults, TArray<FString>& OutErrorMessages)
{
	...
	if(InFiles.Num() > ArcGitSourceControlConstants::MaxFilesPerBatch)
	{
		...
		{
			TArray<FString> FilesInBatch;
			for(int32 FileIndex = 0; FileIndex < ArcGitSourceControlConstants::MaxFilesPerBatch; FileIndex++, FileCount++)
			{
				FilesInBatch.Add(InFiles[FileCount]);
			}
...
public:
	FGitStatusParser(const FString& InResult)
	{
		   ...
		   || (IndexState == 'D' && WCopyState == 'D'))
		{
			...
			State = EArcGitState::Conflicted;
		}
		else if(IndexState == 'A')
		{
			State = EArcGitState::Added;
		}
		else if(IndexState == 'D')
		{
			State = EArcGitState::Deleted;
		}
		else if(WCopyState == 'D')
		{
			State = EArcGitState::Missing;
		}
		else if(IndexState == 'M' || WCopyState == 'M')
		{
			State = EArcGitState::Modified;
		}
		else if(IndexState == 'R')
		{
			State = EArcGitState::Renamed;
		}
		else if(IndexState == 'C')
		{
			State = EArcGitState::Copied;
		}
		else if(IndexState == '?' || WCopyState == '?')
		{
			State = EArcGitState::NotControlled;
		}
		else if(IndexState == '!' || WCopyState == '!')
		{
			State = EArcGitState::Ignored;
		}
		else
		{
			// Unmodified never yield a status
			State = EArcGitState::Unknown;
		}
	}

	EArcGitState::Type State;
};
...
static void RunGetConflictStatus(const FString& InPathToGitBinary, const FString& InRepositoryRoot, const FString& InFile, FArcGitSourceControlState& InOutFileState)
{
	TArray<FString> ErrorMessages;
...
!! BasicCode.sln
*/
static void ParseFileStatusResult(const FString& InPathToGitBinary, const FString& InRepositoryRoot, const TArray<FString>& InFiles, const TArray<FString>& InResults, TArray<FArcGitSourceControlState>& OutStates)
{
	...
	for(const auto& File : InFiles)
	{
		FArcGitSourceControlState FileState(File);
...
*/
static void ParseDirectoryStatusResult(const FString& InPathToGitBinary, const FString& InRepositoryRoot, const TArray<FString>& InResults, TArray<FArcGitSourceControlState>& OutStates)
{
	...
	for(const FString& Result : InResults)
	{
		...
		if((EArcGitState::Deleted == StatusParser.State) || (EArcGitState::Missing == StatusParser.State) || (EArcGitState::NotControlled == StatusParser.State))
		{
			FileState.WorkingCopyState = StatusParser.State;
			FileState.TimeStamp.Now();
 ...
 */
static void ParseStatusResults(const FString& InPathToGitBinary, const FString& InRepositoryRoot, const TArray<FString>& InFiles, const TArray<FString>& InResults, TArray<FArcGitSourceControlState>& OutStates)
{
...
bool RunUpdateStatus(const FString& InPathToGitBinary, const FString& InRepositoryRoot, const TArray<FString>& InFiles, TArray<FString>& OutErrorMessages, TArray<FArcGitSourceControlState>& OutStates)
{
	bool bResults = true;
	TArray<FString> Results;
	TArray<FString> Parameters;
...
bool RunDumpToFile(const FString& InPathToGitBinary, const FString& InRepositoryRoot, const FString& InParameter, const FString& InDumpFileName)
{
	...
	const FArcGitVersion& GitVersion = GitSourceControl.GetProvider().GetGitVersion();
	...
	if(ProcessHandle.IsValid())
	{
		...
		while(FPlatformProcess::IsProcRunning(ProcessHandle))
		{
			...
			if(FPlatformProcess::ReadPipeToArray(PipeRead, BinaryData))
			{
				BinaryFileContent.Append(MoveTemp(BinaryData));
			}
		}
		TArray<uint8> BinaryData;
		while(FPlatformProcess::ReadPipeToArray(PipeRead, BinaryData))
		{
			BinaryFileContent.Append(MoveTemp(BinaryData));
		}
...
*/
static void ParseLogResults(const TArray<FString>& InResults, TArcGitSourceControlHistory& OutHistory)
{
	TSharedRef<FArcGitSourceControlRevision, ESPMode::ThreadSafe> SourceControlRevision = MakeShareable(new FArcGitSourceControlRevision);
	for(const auto& Result : InResults)
	{
		if(Result.StartsWith(TEXT("commit "))) // Start of a new commit
		{
			...
			if(SourceControlRevision->RevisionNumber != 0)
			{
				...
				SourceControlRevision = MakeShareable(new FArcGitSourceControlRevision);
			}
...
bool RunGetHistory(const FString& InPathToGitBinary, const FString& InRepositoryRoot, const FString& InFile, bool bMergeConflict, TArray<FString>& OutErrorMessages, TArcGitSourceControlHistory& OutHistory)
{
	bool bResults;
	{
		TArray<FString> Results;
...
bool UpdateCachedStates(const TArray<FArcGitSourceControlState>& InStates)
{
	FArcGitSourceControlModule& GitSourceControl = FModuleManager::LoadModuleChecked<FArcGitSourceControlModule>( "ArcGitSourceControl" );
	...
	{
		TSharedRef<FArcGitSourceControlState, ESPMode::ThreadSafe> State = Provider.GetStateInternal(InState.LocalFilename);
...
void RemoveRedundantErrors(FArcGitSourceControlCommand& InCommand, const FString& InFilter)
{
...
void ParseGitAttributesForLfs(const FString& RepoRoot, TArray<FString>& OutLfsPatterns)
{
	FString GitAttributesPath = FPaths::Combine(RepoRoot, TEXT(".gitattributes"));
	FString FileContents;
	if (FFileHelper::LoadFileToString(FileContents, *GitAttributesPath))
	{
		...
		{
			if (Line.Contains(TEXT("filter=lfs")))
			{
				// Extract the pattern (text before the first space)
				FString TrimmedLine = Line.TrimStart();
				int32 SpaceIndex;
				if (TrimmedLine.FindChar(TEXT(' '), SpaceIndex))
				{
					FString Pattern = TrimmedLine.Left(SpaceIndex);
					if (!Pattern.IsEmpty())
					{
						OutLfsPatterns.Add(Pattern);
					}
				}
			}
		}
...
bool IsFileLfsTracked(const FString& Filename, const TArray<FString>& LfsPatterns)
{
	for (const FString& Pattern : LfsPatterns)
	{
...
bool RunGetLocks(const FString& InPathToGitBinary, const FString& InRepositoryRoot, TMap<FString, FArcGitLockInfo>& OutLocks, TArray<FString>& OutErrorMessages)
{
	TArray<FString> Results;
	TArray<FString> Parameters;
	Parameters.Add(TEXT("locks"));

	bool bResult = RunCommand(TEXT("lfs"), InPathToGitBinary, InRepositoryRoot, Parameters, TArray<FString>(), Results, OutErrorMessages);

	if (bResult)
	{
		// Parse output lines like: "Content/Maps/Level.umap\tuser@host\tID:123"
		for (const FString& Line : Results)
		{
			FArcGitLockInfo LockInfo;
			FString Remaining;

			if (Line.Split(TEXT("\t"), &LockInfo.Path, &Remaining))
			{
				FString IdStr;
				if (Remaining.Split(TEXT("\t"), &LockInfo.Owner, &IdStr))
				{
					// Parse ID:NNN
					IdStr.RemoveFromStart(TEXT("ID:"));
					LockInfo.LockId = FCString::Atoi(*IdStr);
				}
				else
				{
					LockInfo.Owner = Remaining;
				}

				// Normalize path to absolute
				LockInfo.Path = FPaths::ConvertRelativePathToFull(InRepositoryRoot / LockInfo.Path);
				OutLocks.Add(LockInfo.Path, LockInfo);
			}
		}
	}
...
bool RunCommandWithEnv(const FString& InCommand, const FString& InPathToGitBinary, const FString& InRepositoryRoot, const TArray<FString>& InParameters, const TArray<FString>& InFiles, const TMap<FString, FString>& InEnvVars, TArray<FString>& OutResults, TArray<FString>& OutErrorMessages)
{
	// Save current env vars and set new ones
	TMap<FString, FString> SavedEnvVars;
	for (const auto& EnvVar : InEnvVars)
	{
		FString CurrentValue = FPlatformMisc::GetEnvironmentVariable(*EnvVar.Key);
		SavedEnvVars.Add(EnvVar.Key, CurrentValue);
		FPlatformMisc::SetEnvironmentVar(*EnvVar.Key, *EnvVar.Value);
	}

	// Run the actual command
	bool bResult = RunCommand(InCommand, InPathToGitBinary, InRepositoryRoot, InParameters, InFiles, OutResults, OutErrorMessages);

	// Restore previous env vars
	for (const auto& SavedVar : SavedEnvVars)
	{
		FPlatformMisc::SetEnvironmentVar(*SavedVar.Key, *SavedVar.Value);
	}

	return bResult;
}

FString GetChangelistStoragePath(const FString& InRepositoryRoot)
{
	return FPaths::Combine(InRepositoryRoot, TEXT(".git"), TEXT("arc-changelists"));
}

bool LoadChangelist(const FString& InRepositoryRoot, int32 InId, FString& OutDescription, TArray<FString>& OutFiles, FDateTime& OutCreated)
{
	const FString FilePath = FPaths::Combine(GetChangelistStoragePath(InRepositoryRoot), FString::Printf(TEXT("%d.json"), InId));
	FString FileContents;
	...
	OutDescription = JsonObject->GetStringField(TEXT("description"));
	...
	FString CreatedStr;
	if (JsonObject->TryGetStringField(TEXT("created"), CreatedStr))
	{
		FDateTime::ParseIso8601(*CreatedStr, OutCreated);
	}
	else
	{
		OutCreated = FDateTime::Now();
	}

	return true;
}

bool SaveChangelist(const FString& InRepositoryRoot, int32 InId, const FString& InDescription, const TArray<FString>& InFiles, const FDateTime& InCreated)
{
	const FString StoragePath = GetChangelistStoragePath(InRepositoryRoot);
	...
	JsonObject->SetNumberField(TEXT("id"), InId);
	JsonObject->SetStringField(TEXT("description"), InDescription);
	...
	JsonObject->SetStringField(TEXT("created"), InCreated.ToIso8601());

	FString OutputString;
	TSharedRef<TJsonWriter<>> Writer = TJsonWriterFactory<>::Create(&OutputString);
	FJsonSerializer::Serialize(JsonObject, Writer);

	const FString FilePath = FPaths::Combine(StoragePath, FString::Printf(TEXT("%d.json"), InId));
	return FFileHelper::SaveStringToFile(OutputString, *FilePath, FFileHelper::EEncodingOptions::ForceUTF8WithoutBOM);
}

bool DeleteChangelistFile(const FString& InRepositoryRoot, int32 InId)
{
	const FString FilePath = FPaths::Combine(GetChangelistStoragePath(InRepositoryRoot), FString::Printf(TEXT("%d.json"), InId));
	IPlatformFile& PlatformFile = FPlatformFileManager::Get().GetPlatformFile();
...
TArray<int32> GetAllChangelistIds(const FString& InRepositoryRoot)
{
	TArray<int32> Ids;
	const FString StoragePath = GetChangelistStoragePath(InRepositoryRoot);
	IPlatformFile& PlatformFile = FPlatformFileManager::Get().GetPlatformFile();

	PlatformFile.IterateDirectory(*StoragePath, [&Ids](const TCHAR* FilenameOrDirectory, bool bIsDirectory) -> bool
	{
		if (!bIsDirectory)
		{
			FString Filename = FPaths::GetBaseFilename(FilenameOrDirectory);
			FString Extension = FPaths::GetExtension(FilenameOrDirectory);
			if (Extension == TEXT("json") && Filename != TEXT("meta"))
			{
				int32 Id = FCString::Atoi(*Filename);
				if (Id > 0 || Filename == TEXT("0"))
				{
					Ids.Add(Id);
				}
			}
...
int32 AllocateNextChangelistId(const FString& InRepositoryRoot)
{
	const FString StoragePath = GetChangelistStoragePath(InRepositoryRoot);
	IPlatformFile& PlatformFile = FPlatformFileManager::Get().GetPlatformFile();
	if (!PlatformFile.DirectoryExists(*StoragePath))
	...
	const FString MetaPath = FPaths::Combine(StoragePath, TEXT("meta.json"));
	int32 NextId = 1;

	FString FileContents;
	if (FFileHelper::LoadFileToString(FileContents, *MetaPath))
	{
		TSharedPtr<FJsonObject> JsonObject;
		TSharedRef<TJsonReader<>> Reader = TJsonReaderFactory<>::Create(FileContents);
		if (FJsonSerializer::Deserialize(Reader, JsonObject) && JsonObject.IsValid())
		{
			NextId = static_cast<int32>(JsonObject->GetNumberField(TEXT("nextId")));
		}
	}

	const int32 AllocatedId = NextId;

	// Write back incremented nextId
	TSharedRef<FJsonObject> MetaObject = MakeShared<FJsonObject>();
	MetaObject->SetNumberField(TEXT("nextId"), NextId + 1);

	FString OutputString;
	TSharedRef<TJsonWriter<>> Writer = TJsonWriterFactory<>::Create(&OutputString);
	FJsonSerializer::Serialize(MetaObject, Writer);
	FFileHelper::SaveStringToFile(OutputString, *MetaPath, FFileHelper::EEncodingOptions::ForceUTF8WithoutBOM);

	return AllocatedId;
}

bool RunBlame(const FString& InPathToGitBinary, const FString& InRepositoryRoot,
	const FString& InRevision, const FString& InFile,
	TArray<FAnnotationLine>& OutLines, TArray<FString>& OutErrorMessages)
{
	TArray<FString> Results;
	TArray<FString> Parameters;
	Parameters.Add(TEXT("--porcelain"));
	...
	bool bResult = RunCommand(TEXT("blame"), InPathToGitBinary, InRepositoryRoot,
		Parameters, TArray<FString>(), Results, OutErrorMessages);

	if (bResult)
	{
		FString CurrentCommit;
		FString CurrentAuthor;

		for (const FString& Line : Results)
		{
			if (Line.Len() >= 40 && !Line.StartsWith(TEXT("\t")))
			{
				// Commit line: "<40-char-hash> <orig-line> <final-line> <num-lines>"
				CurrentCommit = Line.Left(8);
			}
			else if (Line.StartsWith(TEXT("author ")))
			{
				CurrentAuthor = Line.RightChop(7);
			}
			else if (Line.StartsWith(TEXT("\t")))
			{
				// Content line
				FAnnotationLine AnnotationLine(FCString::Strtoi(*CurrentCommit, nullptr, 16), CurrentAuthor, Line.RightChop(1));
...
bool RunBlameToFile(const FString& InPathToGitBinary, const FString& InRepositoryRoot,
	const FString& InRevision, const FString& InFile,
	FString& OutTempFilename, TArray<FString>& OutErrorMessages)
{
	TArray<FString> Results;
	TArray<FString> Parameters;
	Parameters.Add(InRevision);
	...
	if (bResult)
	{
		OutTempFilename = FPaths::CreateTempFilename(*FPaths::ProjectSavedDir(), TEXT("ArcGitBlame"));
		FFileHelper::SaveStringArrayToFile(Results, *OutTempFilename);
	}
	return bResult;
}

bool CreateShelveCommit(const FString& InPathToGitBinary, const FString& InRepositoryRoot,
	const TArray<FString>& InFiles, const FString& InDescription,
	FString& OutCommitSha, TArray<FString>& OutErrorMessages)
{
	// 1. Create a temporary index file
	const FString TempIndex = FPaths::CreateTempFilename(*FPaths::ProjectSavedDir(), TEXT("ArcGitShelf"));
	const TMap<FString, FString> EnvVars = {{ TEXT("GIT_INDEX_FILE"), TempIndex }};

	// 2. Seed the temp index from HEAD
	{
		TArray<FString> Results;
		if (!RunCommandWithEnv(TEXT("read-tree"), InPathToGitBinary, InRepositoryRoot,
			TArray<FString>(), { TEXT("HEAD") }, EnvVars, Results, OutErrorMessages))
		{
			IFileManager::Get().Delete(*TempIndex);
			return false;
		}
	}

	// 3. Stage each file into the temp index
	for (const FString& File : InFiles)
	{
		TArray<FString> Results;
		...
		Params.Add(TEXT("--add"));
		Params.Add(TEXT("--"));
		if (!RunCommandWithEnv(TEXT("update-index"), InPathToGitBinary, InRepositoryRoot,
			Params, { File }, EnvVars, Results, OutErrorMessages))
		{
			IFileManager::Get().Delete(*TempIndex);
			return false;
		}
	...
	{
		TArray<FString> Results;
		if (!RunCommandWithEnv(TEXT("write-tree"), InPathToGitBinary, InRepositoryRoot,
			TArray<FString>(), TArray<FString>(), EnvVars, Results, OutErrorMessages) || Results.Num() == 0)
		{
			IFileManager::Get().Delete(*TempIndex);
			return false;
		}
		TreeSha = Results[0].TrimStartAndEnd();
	}

	// 5. Create a commit object pointing at the tree, parented on HEAD
	{
		TArray<FString> Results;
		TArray<FString> Params;
		Params.Add(TreeSha);
		...
		Params.Add(TEXT("-m"));
		Params.Add(InDescription);
		if (!RunCommand(TEXT("commit-tree"), InPathToGitBinary, InRepositoryRoot,
			Params, TArray<FString>(), Results, OutErrorMessages) || Results.Num() == 0)
		{
			IFileManager::Get().Delete(*TempIndex);
			return false;
		}
		OutCommitSha = Results[0].TrimStartAndEnd();
	}
