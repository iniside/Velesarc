...
void UArcAbilityTask::OnDestroy(bool bInOwnerFinished)
{
	if (Ability && AbilitySystemComponent.IsValid())
	{
		if (InstanceName != NAME_None)
		{
			UArcCoreAbilitySystemComponent* ASC = Cast<UArcCoreAbilitySystemComponent>(
				Ability->GetAbilitySystemComponentFromActorInfo());

			ASC->PooledTasks.FindOrAdd(Ability).Remove(InstanceName);
		}
	}
