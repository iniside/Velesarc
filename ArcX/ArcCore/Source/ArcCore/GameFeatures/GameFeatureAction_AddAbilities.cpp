...
#include "Player/ArcCorePlayerState.h" //@TODO: For the fname
...
EDataValidationResult UGameFeatureAction_AddAbilities::IsDataValid(class FDataValidationContext& Context) const
{
	...
	for (const FGameFeatureAbilitiesEntry& Entry : AbilitiesList)
	{
		if (Entry.ActorClass.IsNull())
		{
			...
			Context.AddError(FText::Format(LOCTEXT("EntryHasNullActor"
					, "Null ActorClass at index {0} in AbilitiesList")
		...
		if (Entry.GrantedAbilities.IsEmpty() && Entry.GrantedAttributes.IsEmpty() && Entry.GrantedAbilitySets.IsEmpty())
		{
			...
			Context.AddError(FText::Format(LOCTEXT("EntryHasNoAddOns"
					, "Index {0} in AbilitiesList will do nothing (no granted abilities, attributes, or ability sets)")
				, FText::AsNumber(EntryIndex)));
		}
		...
		for (const FLyraAbilityGrant& Ability : Entry.GrantedAbilities)
		{
			...
			{
				Result = EDataValidationResult::Invalid;
				Context.AddError(FText::Format(LOCTEXT("EntryHasNullAbility"
		...
		for (const FLyraAttributeSetGrant& Attributes : Entry.GrantedAttributes)
		{
			...
			{
				Result = EDataValidationResult::Invalid;
				Context.AddError(FText::Format(LOCTEXT("EntryHasNullAttributeSet"
						, "Null AttributeSetType at index {0} in AbilitiesList[{1}].GrantedAttributes")
					, FText::AsNumber(AttributesIndex)
					, FText::AsNumber(EntryIndex)));
			}
			++AttributesIndex;
		}
		...
		for (const TSoftObjectPtr<const UArcAbilitySet>& AttributeSetPtr : Entry.GrantedAbilitySets)
		{
			if (AttributeSetPtr.IsNull())
			{
				...
				Context.AddError(FText::Format(LOCTEXT("EntryHasNullAttributeSet"
					...
					, FText::AsNumber(AttributeSetIndex)
					, FText::AsNumber(EntryIndex)));
			}
			++AttributeSetIndex;
		}
				...
				EventName == UGameFrameworkComponentManager::NAME_ReceiverRemoved))
		{
			RemoveActorAbilities(Actor
				, *ActiveData);
					 ...
					 EventName == AArcCorePlayerState::NAME_ArcAbilityReady))
		{
			AddActorAbilities(Actor
				, Entry
				, *ActiveData);
		}
	}
														...
														, FPerContextData& ActiveData)
{
	...
	if (UAbilitySystemComponent* AbilitySystemComponent = FindOrAddComponentForActor<UAbilitySystemComponent>(Actor
		, AbilitiesEntry
		, ActiveData))
	{
		...
		UArcCoreAbilitySystemComponent* ArcASC = CastChecked<UArcCoreAbilitySystemComponent>(AbilitySystemComponent);
		for (const TSoftObjectPtr<const UArcAbilitySet>& SetPtr : AbilitiesEntry.GrantedAbilitySets)
		{
			if (const UArcAbilitySet* Set = SetPtr.Get())
			{
				Set->GiveToAbilitySystem(ArcASC
					, &AddedExtensions.AbilitySetHandles.AddDefaulted_GetRef());
			}
		}

		ActiveData.ActiveExtensions.Add(Actor
			, AddedExtensions);
	}
	else
	{
														   ...
														   , FPerContextData& ActiveData)
{
	if (FActorExtensions* ActorExtensions = ActiveData.ActiveExtensions.Find(Actor))
	{
		if (UAbilitySystemComponent* AbilitySystemComponent = Actor->FindComponentByClass<UAbilitySystemComponent>())
		{
			...
			UArcCoreAbilitySystemComponent* ArcASC = CastChecked<
				UArcCoreAbilitySystemComponent>(AbilitySystemComponent);
			for (FArcAbilitySet_GrantedHandles& SetHandle : ActorExtensions->AbilitySetHandles)
			{
				SetHandle.TakeFromAbilitySystem(ArcASC);
			}
		}
...
UActorComponent* UGameFeatureAction_AddAbilities::FindOrAddComponentForActor(UClass* ComponentType
																			 , AActor* Actor
																			 , const FGameFeatureAbilitiesEntry&
																			 AbilitiesEntry
																			 , FPerContextData& ActiveData)
{
	...
	if (Component)
	{
		// Check to see if this component was created from a different
		// `UGameFrameworkComponentManager` request. `Native` is what `CreationMethod`
		// defaults to for dynamically added components.
		if (Component->CreationMethod == EComponentCreationMethod::Native)
		{
			// Attempt to tell the difference between a true native component and one
			// created by the GameFrameworkComponent system. If it is from the
			// UGameFrameworkComponentManager, then we need to make another request
			// (requests are ref counted).
