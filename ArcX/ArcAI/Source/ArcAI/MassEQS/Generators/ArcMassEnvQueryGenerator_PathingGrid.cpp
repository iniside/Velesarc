...
#include "ArcMassEnvQueryGenerator_PathingGrid.h"

#include "NavigationSystem.h"
#include "NavFilters/NavigationQueryFilter.h"
#include "NavMesh/RecastNavMesh.h"

UArcMassEnvQueryGenerator_PathingGrid::UArcMassEnvQueryGenerator_PathingGrid(const FObjectInitializer& ObjectInitializer)
...
#if WITH_RECAST
namespace Arcx::PathGridHelpers
{
...
void UArcMassEnvQueryGenerator_PathingGrid::ProjectAndFilterNavPoints(TArray<FNavLocation>& Points, FEnvQueryInstance& QueryInstance) const
{
	...
	ARecastNavMesh* NavMeshData = Cast<ARecastNavMesh>(NavMeshSubsystem->GetNavDataForProps(NavAgentProperties, Points[0].Location));
	...
	FSharedConstNavQueryFilter NavQueryFilter = NavigationFilter ? UNavigationQueryFilter::GetQueryFilter(*NavMeshData, NavigationFilter) : NavMeshData->GetDefaultQueryFilter();
	...
	if (NavigationFilterCopy.IsValid())
	{
		...
		for (int32 ContextIdx = 0; ContextIdx < ContextLocations.Num() && Points.Num(); ContextIdx++)
		{
			...
			{
				bool bHasPath = Arcx::PathGridHelpers::HasPath(NodePoolData, Points[Idx].NodeRef);
				if (!bHasPath && Points[Idx].NodeRef != INVALID_NAVNODEREF)
				{
					...
					{
						const bool bHasPathTest = Arcx::PathGridHelpers::HasPath(NodePoolData, HitLocations[HitIdx].NodeRef);
						if (bHasPathTest)
						{
