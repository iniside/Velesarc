...
- `UArcAdvertisementStateTreeSchema` — inherits `UMassStateTreeSchema`. Defines context data: ContextActor (optional), ExecutingEntityHandle, SourceEntityHandle, AdvertisementHandle. Allows Mass nodes (tasks, conditions, evaluators), common nodes, AI nodes, and Actor/Component external data binding. Mass fragment binding available via entity view.
- `FArcAdvertisementExecutionContext` — lightweight StateTree execution wrapper (Activate/Tick/Deactivate). Accepts optional `FMassExecutionContext*` — when provided, creates `FMassStateTreeExecutionContext` internally enabling Mass-specific nodes and entity fragment resolution. Owner is any `UObject*` (typically `UArcKnowledgeSubsystem`), ContextActor is optional.
