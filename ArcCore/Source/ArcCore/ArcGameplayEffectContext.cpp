...
UMassAgentComponent* FArcGameplayEffectContext::GetInstigatorMassAgent() const
{
	if (InstigatorMassAgent)
	{
		return InstigatorMassAgent;
	}

	if (Instigator.IsValid())
	{
		InstigatorMassAgent = Instigator->FindComponentByClass<UMassAgentComponent>();
	}

	if (!InstigatorMassAgent)
	{
		AActor* Causer = GetEffectCauser();
		if (Causer)
		{
			InstigatorMassAgent = Causer->FindComponentByClass<UMassAgentComponent>();
		}
	}
...
UMassEntitySubsystem* FArcGameplayEffectContext::GetMassEntitySubsystem() const
{
	if (CachedMassEntitySubsystem)
	{
		return CachedMassEntitySubsystem;
	}

	AActor* ResolvedActor = GetInstigator();
	if (!ResolvedActor)
	{
		ResolvedActor = GetEffectCauser();
	}

	if (ResolvedActor)
	{
		if (UWorld* World = ResolvedActor->GetWorld())
		{
			CachedMassEntitySubsystem = World->GetSubsystem<UMassEntitySubsystem>();
		}
	}
...
FMassEntityHandle FArcGameplayEffectContext::GetInstigatorEntity() const
{
	if (InstigatorEntity.IsValid())
	{
		return InstigatorEntity;
	}

	UMassAgentComponent* Agent = GetInstigatorMassAgent();
	if (Agent)
	{
		InstigatorEntity = Agent->GetEntityHandle();
	}

	return InstigatorEntity;
}

FMassEntityManager* FArcGameplayEffectContext::GetMassEntityManager() const
{
	UMassEntitySubsystem* Subsystem = GetMassEntitySubsystem();
	if (Subsystem)
	{
