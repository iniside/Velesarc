...
#define ARC_DEFINE_PRE_ATTRIBUTE_HANDLER(AttributeName)                    \
	TFunction<void(const FGameplayAttribute& Attribute, float& NewValue)> \
		Pre##AttributeName##Handler();

#define ARC_DECLARE_PRE_ATTRIBUTE_HANDLER(ClassName, AttributeName)        \
	TFunction<void(const FGameplayAttribute& Attribute, float& NewValue)> \
		ClassName::Pre##AttributeName##Handler()

#define ARC_REGISTER_PRE_ATTRIBUTE_HANDLER(AttributeName)              \
	PreAttributeHandlers.FindOrAdd(Get##AttributeName##Attribute()) = \
		Pre##AttributeName##Handler();
