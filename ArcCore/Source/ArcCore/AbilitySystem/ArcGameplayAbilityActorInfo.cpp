												 ...
												 , UAbilitySystemComponent* InAbilitySystemComponent)
{
	...
	AActor* Source = InAvatarActor ? InAvatarActor : InOwnerActor;
	if (OwnerPawn.IsValid())
	{
		if (!MassAgentComponent)
		{
			MassAgentComponent = OwnerPawn->FindComponentByClass<UMassAgentComponent>();
			EntityHandle = MassAgentComponent ? MassAgentComponent->GetEntityHandle() : FMassEntityHandle();
		}
	...
	if (!MassAgentComponent && InOwnerActor)
	{
		MassAgentComponent = InOwnerActor->FindComponentByClass<UMassAgentComponent>();
		EntityHandle = MassAgentComponent ? MassAgentComponent->GetEntityHandle() : FMassEntityHandle();
	}
	
	if (!MassEntitySubsystem && InOwnerActor)
	{
		if (UWorld* World = InOwnerActor->GetWorld())
		{
			MassEntitySubsystem = World->GetSubsystem<UMassEntitySubsystem>();
		}
	}
