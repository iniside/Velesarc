...
#if WITH_EDITOR
FText FArcMassEntityTagsCondition::GetDescription(const FGuid& ID, FStateTreeDataView InstanceDataView, const IStateTreeBindingLookup& BindingLookup, EStateTreeNodeFormatting Formatting) const
{
	if (InstanceDataView.IsValid())
	{
		...
		if (InstanceData)
		{
			return FText::Format(NSLOCTEXT("ArcAI", "EntityTagsCondDesc", "{0}Entity Has {1} Tags: {2}"),
