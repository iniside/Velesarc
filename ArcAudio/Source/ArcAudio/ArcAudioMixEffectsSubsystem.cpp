...
#include "ArcAudioMixEffectsSubsystem.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(ArcAudioMixEffectsSubsystem)

FArcGetVolumeOptions UArcAudioMixEffectsSubsystem::GetVolumeOptions = FArcGetVolumeOptions();

class FSubsystemCollectionBase;

void UArcAudioMixEffectsSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	Super::Initialize(Collection);
}

void UArcAudioMixEffectsSubsystem::Deinitialize()
{
...
bool UArcAudioMixEffectsSubsystem::ShouldCreateSubsystem(UObject* Outer) const
{
...
void UArcAudioMixEffectsSubsystem::PostInitialize()
{
	Super::PostInitialize();
	
	if (const UArcAudioSettings* LyraAudioSettings = GetDefault<UArcAudioSettings>())
	{
		...
		for (const FArcSubmixEffectChainMap& SoftSubmixEffectChain : LyraAudioSettings->HDRAudioSubmixEffectChain)
		{
			FArcAudioSubmixEffectsChain NewEffectChain;
		...
		for (const FArcSubmixEffectChainMap& SoftSubmixEffectChain : LyraAudioSettings->LDRAudioSubmixEffectChain)
		{
			FArcAudioSubmixEffectsChain NewEffectChain;
...
void UArcAudioMixEffectsSubsystem::OnWorldBeginPlay(UWorld& InWorld)
{
	...
	if (const UWorld* World = InWorld.GetWorld())
	{
		...
		// TODO: Retrieve the user settings
		if (GetVolumeOptions.IsBound())
		{
			FArcVolumeOptions Options = GetVolumeOptions.Execute();
			...
			if (UserMix)
			{
				...
				{
					const FSoundControlBusMixStage OverallControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, OverallControlBus, Options.OverallVolume);
					const FSoundControlBusMixStage MusicControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, MusicControlBus, Options.MusicVolume);
					const FSoundControlBusMixStage SoundFXControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, SoundFXControlBus, Options.SoundFXVolume);
					const FSoundControlBusMixStage DialogueControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, DialogueControlBus, Options.DialogueVolume);
					const FSoundControlBusMixStage VoiceChatControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, VoiceChatControlBus, Options.VoiceChatVolume);
			...
			ApplyDynamicRangeEffectsChains(Options.bUseHDRAudioMode);
		}
	}
}

void UArcAudioMixEffectsSubsystem::ApplyDynamicRangeEffectsChains(bool bHDRAudio)
{
	TArray<FArcAudioSubmixEffectsChain> AudioSubmixEffectsChainToApply;
	TArray<FArcAudioSubmixEffectsChain> AudioSubmixEffectsChainToClear;
	...
	for (const FArcAudioSubmixEffectsChain& EffectChainToClear : AudioSubmixEffectsChainToClear)
	{
		bool bAddToList = true;

		for (const FArcAudioSubmixEffectsChain& SubmixEffectChain : AudioSubmixEffectsChainToApply)
		{
...
void UArcAudioMixEffectsSubsystem::OnLoadingScreenStatusChanged(bool bShowingLoadingScreen)
{
	ApplyOrRemoveLoadingScreenMix(bShowingLoadingScreen);
}

void UArcAudioMixEffectsSubsystem::ApplyOrRemoveLoadingScreenMix(bool bWantsLoadingScreenMix)
{
...
bool UArcAudioMixEffectsSubsystem::DoesSupportWorldType(const EWorldType::Type World) const
{
