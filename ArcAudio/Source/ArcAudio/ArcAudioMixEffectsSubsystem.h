...
#include "ArcAudioMixEffectsSubsystem.generated.h"
...
struct ARCAUDIO_API FArcVolumeOptions
{
	GENERATED_BODY()
...
DECLARE_DELEGATE_RetVal(FArcVolumeOptions, FArcGetVolumeOptions)

USTRUCT()
struct ARCAUDIO_API FArcAudioSubmixEffectsChain
{
	GENERATED_BODY()
 ...
 */
UCLASS()
class ARCAUDIO_API UArcAudioMixEffectsSubsystem : public UWorldSubsystem
{
	GENERATED_BODY()

public:
	static FArcGetVolumeOptions GetVolumeOptions;
...
protected:
	...
	UPROPERTY(Transient)
	TArray<FArcAudioSubmixEffectsChain> HDRSubmixEffectChain;

	// Submix Effect hain Overrides to apply when HDR Audio is turned off
	UPROPERTY(Transient)
	TArray<FArcAudioSubmixEffectsChain> LDRSubmixEffectChain;
