...
namespace Arcx::GameplayDebugger::Abilities
{
...
void FArcGameplayAbilitiesDebugger::DrawAbilitySystemDetails(UAbilitySystemComponent* InASC)
{
	...
	if (ImGui::CollapsingHeader("Overview", ImGuiTreeNodeFlags_DefaultOpen))
	{
		if (ImGui::BeginTable("OverviewTable", 2, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg))
		{
			...
			Arcx::GameplayDebugger::Abilities::TableRow("Class", InASC->GetClass()->GetName());
			Arcx::GameplayDebugger::Abilities::TableRow("Owner Actor", GetNameSafe(InASC->GetOwnerActor()));
			Arcx::GameplayDebugger::Abilities::TableRow("Avatar Actor", GetNameSafe(InASC->GetAvatarActor()));
			Arcx::GameplayDebugger::Abilities::TableRowBool("Is Net Simulated", InASC->IsNetSimulating());
			Arcx::GameplayDebugger::Abilities::TableRowBool("Has Authority", InASC->IsOwnerActorAuthoritative());
			Arcx::GameplayDebugger::Abilities::TableRowFloat("Last Activated Time", InASC->GetAbilityLastActivatedTime());

#if !UE_BUILD_SHIPPING
			if (ArcASC)
			{
				Arcx::GameplayDebugger::Abilities::TableRowInt("Scope Lock Count", ArcASC->GetDebug_AbilityScopeLockCount());
			}
#endif
		...
		for (const FGameplayAbilitySpec& AbilitySpec : ActivatableAbilities)
		{
			...
			if (ImGui::TreeNode(TCHAR_TO_ANSI(*TreeLabel)))
			{
				...
				if (ImGui::BeginTable("AbilityDetails", 2, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg))
				{
					...
					Arcx::GameplayDebugger::Abilities::TableRow("Handle", FString::Printf(TEXT("%s"), *AbilitySpec.Handle.ToString()));
					Arcx::GameplayDebugger::Abilities::TableRow("Ability Class", GetNameSafe(AbilitySpec.Ability ? AbilitySpec.Ability->GetClass() : nullptr));
					Arcx::GameplayDebugger::Abilities::TableRowInt("Level", AbilitySpec.Level);
					Arcx::GameplayDebugger::Abilities::TableRowInt("Input ID", AbilitySpec.InputID);
					Arcx::GameplayDebugger::Abilities::TableRow("Source Object", GetNameSafe(AbilitySpec.SourceObject.Get()));
					Arcx::GameplayDebugger::Abilities::TableRowInt("Active Count", AbilitySpec.ActiveCount);
					Arcx::GameplayDebugger::Abilities::TableRowBool("Input Pressed", AbilitySpec.InputPressed);
					Arcx::GameplayDebugger::Abilities::TableRowBool("Remove After Activation", AbilitySpec.RemoveAfterActivation);
					Arcx::GameplayDebugger::Abilities::TableRowBool("Pending Remove", AbilitySpec.PendingRemove);
					...
					if (Ability)
					{
						...
						Arcx::GameplayDebugger::Abilities::TableRowBool("Is Active", Ability->IsActive());
						Arcx::GameplayDebugger::Abilities::TableRowBool("Is Triggered", Ability->IsTriggered());
						Arcx::GameplayDebugger::Abilities::TableRowBool("Is Predicting Client", Ability->IsPredictingClient());
						Arcx::GameplayDebugger::Abilities::TableRowBool("Is For Remote Client", Ability->IsForRemoteClient());
						Arcx::GameplayDebugger::Abilities::TableRowBool("Can Be Canceled", Ability->CanBeCanceled());
						Arcx::GameplayDebugger::Abilities::TableRowBool("Is Blocking Other Abilities", Ability->IsBlockingOtherAbilities());
						...
						Arcx::GameplayDebugger::Abilities::TableRowBool("Can Activate Ability", bCanActivate);

						const bool bSatisfiesTags = Ability->DoesAbilitySatisfyTagRequirements(*InASC);
						Arcx::GameplayDebugger::Abilities::TableRowBool("Satisfies Tag Requirements", bSatisfiesTags);
						...
						Arcx::GameplayDebugger::Abilities::TableRowTags("Asset Tags", Ability->GetAssetTags());
						...
						if (ArcAbility)
						{
							...
							Arcx::GameplayDebugger::Abilities::TableRowBool("ASC Has Blocking Tags", bASCBlocks);

							Arcx::GameplayDebugger::Abilities::TableRowTags("Activation Required Tags", ArcAbility->GetActivationRequiredTags());
							...
							Arcx::GameplayDebugger::Abilities::TableRowBool("Missing Required Tags", bMissingRequired);

							Arcx::GameplayDebugger::Abilities::TableRowTags("Activation Blocked Tags", ArcAbility->GetActivationBlockedTags());
							...
							Arcx::GameplayDebugger::Abilities::TableRowBool("Has Blocked Tags", bHasBlockedTags);

							Arcx::GameplayDebugger::Abilities::TableRowTags("Activation Owned Tags", ArcAbility->GetActivationOwnedTags());
							Arcx::GameplayDebugger::Abilities::TableRowTags("Cancel Abilities With Tags", ArcAbility->GetCancelAbilitiesWithTag());
							Arcx::GameplayDebugger::Abilities::TableRowTags("Block Abilities With Tags", ArcAbility->GetBlockAbilitiesWithTag());
							Arcx::GameplayDebugger::Abilities::TableRowTags("Source Required Tags", ArcAbility->GetSourceRequiredTags());
							Arcx::GameplayDebugger::Abilities::TableRowTags("Source Blocked Tags", ArcAbility->GetSourceBlockedTags());
							Arcx::GameplayDebugger::Abilities::TableRowTags("Target Required Tags", ArcAbility->GetTargetRequiredTags());
							Arcx::GameplayDebugger::Abilities::TableRowTags("Target Blocked Tags", ArcAbility->GetTargetBlockedTags());
							Arcx::GameplayDebugger::Abilities::TableRowTags("Custom Cooldown Tags", ArcAbility->GetCustomCooldownTags());
							...
							if (ArcASC)
							{
								...
								Arcx::GameplayDebugger::Abilities::TableRowBool("Is On Cooldown", bOnCooldown);
								if (bOnCooldown)
								{
									Arcx::GameplayDebugger::Abilities::TableRowFloat("Cooldown Remaining", ArcASC->GetRemainingTime(AbilitySpec.Handle));
								}

								const double ActivationStartTime = ArcASC->GetAbilityActivationStartTime(AbilitySpec.Handle);
								if (ActivationStartTime >= 0)
								{
									Arcx::GameplayDebugger::Abilities::TableRowFloat("Activation Start Time", static_cast<float>(ActivationStartTime));
									Arcx::GameplayDebugger::Abilities::TableRowFloat("Time Since Activation", static_cast<float>(WorldTime - ActivationStartTime));
								}

								Arcx::GameplayDebugger::Abilities::TableRowBool("Activate On Trigger", ArcAbility->GetActivateOnTrigger());
							}
#endif
							...
							if (UArcItemGameplayAbility* ArcItemAbility = Cast<UArcItemGameplayAbility>(ArcAbility))
							{
								...
								Arcx::GameplayDebugger::Abilities::TableRow("Source Item Def", GetNameSafe(SourceItemDef));

								UArcItemsStoreComponent* SourceItemsStore = ArcItemAbility->GetItemsStoreComponent(nullptr);
								Arcx::GameplayDebugger::Abilities::TableRow("Items Store", GetNameSafe(SourceItemsStore));

								const FArcItemData* ItemData = ArcItemAbility->GetSourceItemEntryPtr();
								Arcx::GameplayDebugger::Abilities::TableRowBool("Has Valid Item Data", ItemData != nullptr);

								const FArcItemId ItemId = ArcItemAbility->GetSourceItemHandle();
								Arcx::GameplayDebugger::Abilities::TableRow("Source Item Id", ItemId.ToString());

								Arcx::GameplayDebugger::Abilities::TableRow("Current Item Slot", ArcItemAbility->GetCurrentItemSlot().ToString());
							}
						}
					}
					else
					{
						...
						if (AbilityCDO)
						{
							Arcx::GameplayDebugger::Abilities::TableRow("CDO Class", AbilityCDO->GetClass()->GetName());
							Arcx::GameplayDebugger::Abilities::TableRowTags("Asset Tags", AbilityCDO->GetAssetTags());
						}
					}
		...
		for (const FActiveGameplayEffectHandle& Handle : AllHandles)
		{
			...
			if (ImGui::TreeNode(TCHAR_TO_ANSI(*EffectLabel)))
			{
				...
				if (ImGui::BeginTable("GEDetails", 2, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg))
				{
					...
					Arcx::GameplayDebugger::Abilities::TableRowBool("Is Inhibited", ActiveGE->bIsInhibited);
					Arcx::GameplayDebugger::Abilities::TableRowInt("Stack Count", ActiveGE->Spec.GetStackCount());
					Arcx::GameplayDebugger::Abilities::TableRowFloat("Duration", ActiveGE->Spec.GetDuration());
					Arcx::GameplayDebugger::Abilities::TableRowFloat("Period", ActiveGE->Spec.GetPeriod());
					Arcx::GameplayDebugger::Abilities::TableRowFloat("Level", ActiveGE->Spec.GetLevel());
					Arcx::GameplayDebugger::Abilities::TableRowFloat("End Time", ActiveGE->GetEndTime());
					Arcx::GameplayDebugger::Abilities::TableRowFloat("Time Remaining", ActiveGE->GetTimeRemaining(WorldTime));

					// Context
					Arcx::GameplayDebugger::Abilities::TableRow("Effect Causer", GetNameSafe(ActiveGE->Spec.GetEffectContext().GetEffectCauser()));
					Arcx::GameplayDebugger::Abilities::TableRow("Instigator", GetNameSafe(ActiveGE->Spec.GetEffectContext().GetInstigator()));
					Arcx::GameplayDebugger::Abilities::TableRow("Source Object", GetNameSafe(ActiveGE->Spec.GetEffectContext().GetSourceObject()));
					Arcx::GameplayDebugger::Abilities::TableRow("Ability", GetNameSafe(ActiveGE->Spec.GetEffectContext().GetAbility()));

					// Tags
					{
						FGameplayTagContainer GrantedTags;
						ActiveGE->Spec.GetAllGrantedTags(GrantedTags);
						Arcx::GameplayDebugger::Abilities::TableRowTags("Granted Tags", GrantedTags);
					}
	...
	if (ImGui::CollapsingHeader("Animation Montages"))
	{
		...
		if (ArcASC)
		{
			...
			if (ImGui::BeginTable("MontageTable", 2, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg))
			{
				...
				Arcx::GameplayDebugger::Abilities::TableRow("Local Anim Montage", GetNameSafe(LocalMontage.AnimMontage));
				Arcx::GameplayDebugger::Abilities::TableRowInt("Play Instance ID", LocalMontage.PlayInstanceId);
				Arcx::GameplayDebugger::Abilities::TableRow("Animating Ability", GetNameSafe(LocalMontage.AnimatingAbility.Get()));
