...
void FArcGameplayEffectsDebugger::DrawEffectsDetails(UAbilitySystemComponent* InASC)
{
	...
	if (ImGui::CollapsingHeader("Overview", ImGuiTreeNodeFlags_DefaultOpen))
	{
		if (ImGui::BeginTable("GEOverview", 2, ImGuiTableFlags_BordersInnerV | ImGuiTableFlags_RowBg))
		{
			...
			Arcx::GameplayDebugger::Effects::TableRow("ASC Class", InASC->GetClass()->GetName());
			Arcx::GameplayDebugger::Effects::TableRow("Owner Actor", GetNameSafe(InASC->GetOwnerActor()));
			Arcx::GameplayDebugger::Effects::TableRow("Avatar Actor", GetNameSafe(InASC->GetAvatarActor()));
			...
			Arcx::GameplayDebugger::Effects::TableRowInt("Total Active Effects", AllHandles.Num());
			...
			Arcx::GameplayDebugger::Effects::TableRowInt("Inhibited Effects", InhibitedCount);

			Arcx::GameplayDebugger::Effects::TableRowInt("Attribute Sets", InASC->GetSpawnedAttributes().Num());
		...
		for (const FActiveGameplayEffectHandle& Handle : AllHandles)
		{
			...
			if (ImGui::TreeNode(TCHAR_TO_ANSI(*EffectLabel)))
			{
				...
				if (ImGui::BeginTable("GEBasic", 2, ImGuiTableFlags_BordersInnerV | ImGuiTableFlags_RowBg))
				{
					...
					Arcx::GameplayDebugger::Effects::TableRow("Effect Class", EffectName);
					Arcx::GameplayDebugger::Effects::TableRowBool("Is Inhibited", bInhibited);
					Arcx::GameplayDebugger::Effects::TableRowInt("Stack Count", ActiveGE->Spec.GetStackCount());

					if (GEDef)
					{
						Arcx::GameplayDebugger::Effects::TableRow("Duration Policy", FString(Arcx::GameplayDebugger::Effects::DurationPolicyToString(GEDef->DurationPolicy)));
					}

					Arcx::GameplayDebugger::Effects::TableRowFloat("Duration", ActiveGE->Spec.GetDuration());
					Arcx::GameplayDebugger::Effects::TableRowFloat("Period", ActiveGE->Spec.GetPeriod());
					Arcx::GameplayDebugger::Effects::TableRowFloat("Level", ActiveGE->Spec.GetLevel());

					float TimeRemaining = ActiveGE->GetTimeRemaining(WorldTime);
					if (ActiveGE->Spec.GetDuration() > 0.0f)
					{
						Arcx::GameplayDebugger::Effects::TableRowFloat("Time Remaining", TimeRemaining);
						Arcx::GameplayDebugger::Effects::TableRowFloat("End Time", ActiveGE->GetEndTime());
					}

					Arcx::GameplayDebugger::Effects::TableRowFloat("Start World Time", ActiveGE->StartWorldTime);
					Arcx::GameplayDebugger::Effects::TableRowFloat("Start Server World Time", ActiveGE->StartServerWorldTime);
					Arcx::GameplayDebugger::Effects::TableRowBool("Pending Remove", ActiveGE->IsPendingRemove);
				...
				if (GEDef && GEDef->GetStackingType() != EGameplayEffectStackingType::None)
				{
					if (ImGui::TreeNode("Stacking"))
					{
						if (ImGui::BeginTable("GEStacking", 2, ImGuiTableFlags_BordersInnerV | ImGuiTableFlags_RowBg))
						{
							...
							Arcx::GameplayDebugger::Effects::TableRow("Stacking Type", FString(Arcx::GameplayDebugger::Effects::StackingTypeToString(GEDef->GetStackingType())));
							Arcx::GameplayDebugger::Effects::TableRowInt("Stack Limit", GEDef->GetStackLimitCount());
							Arcx::GameplayDebugger::Effects::TableRowInt("Current Stacks", ActiveGE->Spec.GetStackCount());
							Arcx::GameplayDebugger::Effects::TableRow("Duration Refresh", *UEnum::GetValueAsString(GEDef->StackDurationRefreshPolicy));
							Arcx::GameplayDebugger::Effects::TableRow("Period Reset", *UEnum::GetValueAsString(GEDef->StackPeriodResetPolicy));
							Arcx::GameplayDebugger::Effects::TableRow("Expiration Policy", *UEnum::GetValueAsString(GEDef->StackExpirationPolicy));
				...
				if (ImGui::TreeNode("Tags"))
				{
					...
					Arcx::GameplayDebugger::Effects::TableRowTags("Granted Tags", GrantedTags);

					FGameplayTagContainer BlockedTags;
					ActiveGE->Spec.GetAllBlockedAbilityTags(BlockedTags);
					Arcx::GameplayDebugger::Effects::TableRowTags("Blocked Ability Tags", BlockedTags);

					FGameplayTagContainer AssetTags;
					ActiveGE->Spec.GetAllAssetTags(AssetTags);
					Arcx::GameplayDebugger::Effects::TableRowTags("Asset Tags", AssetTags);

					Arcx::GameplayDebugger::Effects::TableRowTags("Dynamic Granted Tags", ActiveGE->Spec.DynamicGrantedTags);
				...
				if (GEDef)
				{
					if (const UTargetTagRequirementsGameplayEffectComponent* TagReqComp = GEDef->FindComponent<UTargetTagRequirementsGameplayEffectComponent>())
					{
						...
						{
							Arcx::GameplayDebugger::Effects::DrawTagRequirements("Application Requirements", TagReqComp->ApplicationTagRequirements);
							Arcx::GameplayDebugger::Effects::DrawTagRequirements("Ongoing Requirements", TagReqComp->OngoingTagRequirements);
							Arcx::GameplayDebugger::Effects::DrawTagRequirements("Removal Requirements", TagReqComp->RemovalTagRequirements);
						...
						for (int32 Idx = 0; Idx < GEDef->Modifiers.Num(); Idx++)
						{
							...
							if (ImGui::TreeNode(TCHAR_TO_ANSI(*ModLabel)))
							{
								if (ImGui::BeginTable("ModDetail", 2, ImGuiTableFlags_BordersInnerV | ImGuiTableFlags_RowBg))
								{
									...
									Arcx::GameplayDebugger::Effects::TableRow("Attribute", ModInfo.Attribute.GetName());
									Arcx::GameplayDebugger::Effects::TableRow("Operation", *UEnum::GetValueAsString(ModInfo.ModifierOp));
									Arcx::GameplayDebugger::Effects::TableRowFloat("Magnitude", Magnitude);
									Arcx::GameplayDebugger::Effects::TableRow("Evaluation Channel", *UEnum::GetValueAsString(ModInfo.EvaluationChannelSettings.GetEvaluationChannel()));

									ImGui::EndTable();
								}

								Arcx::GameplayDebugger::Effects::DrawTagRequirements("Source Tags", ModInfo.SourceTags);
								Arcx::GameplayDebugger::Effects::DrawTagRequirements("Target Tags", ModInfo.TargetTags);
				...
				if (ImGui::TreeNode("Effect Context"))
				{
					...
					if (CtxHandle.IsValid())
					{
						if (ImGui::BeginTable("GEContext", 2, ImGuiTableFlags_BordersInnerV | ImGuiTableFlags_RowBg))
						{
							...
							Arcx::GameplayDebugger::Effects::TableRow("Effect Causer", GetNameSafe(CtxHandle.GetEffectCauser()));
							Arcx::GameplayDebugger::Effects::TableRow("Instigator", GetNameSafe(CtxHandle.GetInstigator()));
							Arcx::GameplayDebugger::Effects::TableRow("Original Instigator", GetNameSafe(CtxHandle.GetOriginalInstigator()));
							Arcx::GameplayDebugger::Effects::TableRow("Source Object", GetNameSafe(CtxHandle.GetSourceObject()));
							Arcx::GameplayDebugger::Effects::TableRow("Ability", GetNameSafe(CtxHandle.GetAbility()));
							Arcx::GameplayDebugger::Effects::TableRowInt("Ability Level", CtxHandle.GetAbilityLevel());
						...
						if (CtxHandle.Get() && CtxHandle.Get()->GetScriptStruct()->IsChildOf(FArcGameplayEffectContext::StaticStruct()))
						{
							...
							if (ImGui::TreeNode("Arc Context"))
							{
								if (ImGui::BeginTable("ArcCtx", 2, ImGuiTableFlags_BordersInnerV | ImGuiTableFlags_RowBg))
								{
									...
									Arcx::GameplayDebugger::Effects::TableRow("Source Item Handle", ArcCtx->GetSourceItemHandle().ToString());
									Arcx::GameplayDebugger::Effects::TableRowBool("Has Valid Item Data", ArcCtx->GetSourceItemPtr() != nullptr);
									Arcx::GameplayDebugger::Effects::TableRow("Source Item Def", GetNameSafe(ArcCtx->GetSourceItem()));
				...
				if (GEDef && ImGui::TreeNode("GE Definition"))
				{
					if (ImGui::BeginTable("GEDefTable", 2, ImGuiTableFlags_BordersInnerV | ImGuiTableFlags_RowBg))
					{
						...
						Arcx::GameplayDebugger::Effects::TableRow("Class", GEDef->GetClass()->GetName());
						Arcx::GameplayDebugger::Effects::TableRow("Duration Policy", FString(Arcx::GameplayDebugger::Effects::DurationPolicyToString(GEDef->DurationPolicy)));
						Arcx::GameplayDebugger::Effects::TableRow("Stacking Type", FString(Arcx::GameplayDebugger::Effects::StackingTypeToString(GEDef->GetStackingType())));
						Arcx::GameplayDebugger::Effects::TableRowInt("Stack Limit", GEDef->GetStackLimitCount());
						Arcx::GameplayDebugger::Effects::TableRowInt("Modifier Count", GEDef->Modifiers.Num());
						Arcx::GameplayDebugger::Effects::TableRowInt("Execution Count", GEDef->Executions.Num());

						ImGui::EndTable();
					}

					// GE Definition tags
					Arcx::GameplayDebugger::Effects::TableRowTags("Asset Tags (Def)", GEDef->GetAssetTags());
					Arcx::GameplayDebugger::Effects::TableRowTags("Granted Tags (Def)", GEDef->GetGrantedTags());
					Arcx::GameplayDebugger::Effects::TableRowTags("Blocked Ability Tags (Def)", GEDef->GetBlockedAbilityTags());
