...
EDataValidationResult UArcRecipeDefinition::IsDataValid(FDataValidationContext& Context) const
{
	...
	for (int32 Idx = 0; Idx < OutputModifiers.Num(); ++Idx)
	{
		const FArcRecipeOutputModifier* Modifier = OutputModifiers[Idx].GetPtr<FArcRecipeOutputModifier>();
		if (Modifier)
		{
			if (Modifier->IsDataValid(Context) == EDataValidationResult::Invalid)
			{
				Result = EDataValidationResult::Invalid;
			}
		}
