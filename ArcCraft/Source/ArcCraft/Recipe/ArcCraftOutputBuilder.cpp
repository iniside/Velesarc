...
static void ApplyResult(FArcItemSpec& OutSpec, const FArcCraftModifierResult& Result)
{
	...
	case EArcCraftModifierResultType::AttributeModifier:
		{
			if (Result.AttributeModifierEffect)
			{
				FArcItemFragment_AttributeModifier* Fragment =
					OutSpec.FindFragmentMutable<FArcItemFragment_AttributeModifier>();
				if (!Fragment)
				{
					Fragment = new FArcItemFragment_AttributeModifier();
					Fragment->BackingGameplayEffect = Result.AttributeModifierEffect;
					Fragment->Attributes = Result.AttributeModifierValues;
					OutSpec.AddInstanceData(Fragment);
				}
				else if (Fragment->BackingGameplayEffect == Result.AttributeModifierEffect)
				{
					for (const auto& [Tag, Magnitude] : Result.AttributeModifierValues)
					{
						Fragment->Attributes.Add(Tag, Magnitude);
					}
				}
			}
		}
