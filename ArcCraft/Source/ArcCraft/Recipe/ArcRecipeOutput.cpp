	...
	float EffectiveQuality)
{
	...
	for (const FInstancedStruct& ModStruct : Modifiers)
	{
		...
		// AttributeModifier
		if (const FArcCraftModifier_AttributeModifier* AttrMod = ModStruct.GetPtr<FArcCraftModifier_AttributeModifier>())
		{
			if (AttrMod->BackingGameplayEffect)
			{
				const float QScale = 1.0f + (EffectiveQuality - 1.0f) * AttrMod->QualityScalingFactor;
				FArcCraftModifierResult R;
				R.Type = EArcCraftModifierResultType::AttributeModifier;
				R.AttributeModifierEffect = AttrMod->BackingGameplayEffect;
				for (const auto& [Tag, Magnitude] : AttrMod->BaseAttributes)
				{
					R.AttributeModifierValues.Add(Tag, Magnitude * QScale);
				}
				Results.Add(MoveTemp(R));
			}
			continue;
...
// -------------------------------------------------------------------
// FArcRecipeOutputModifier_AttributeModifier
// -------------------------------------------------------------------

TArray<FArcCraftPendingModifier> FArcRecipeOutputModifier_AttributeModifier::Evaluate(
	const TArray<FArcItemSpec>& ConsumedIngredients,
	const TArray<float>& IngredientQualityMults,
	float AverageQuality) const
{
	if (!IsEligible(ConsumedIngredients, AverageQuality) || !BackingGameplayEffect)
	{
		return {};
	}
	...
	Pending.Result.Type = EArcCraftModifierResultType::AttributeModifier;
	Pending.Result.AttributeModifierEffect = BackingGameplayEffect;
	for (const auto& [Tag, Magnitude] : BaseAttributes)
	{
		Pending.Result.AttributeModifierValues.Add(Tag, Magnitude * QualityScale);
	}
...
EDataValidationResult FArcRecipeOutputModifier::IsDataValid(FDataValidationContext& Context) const
{
	return EDataValidationResult::Valid;
}

EDataValidationResult FArcRecipeOutputModifier_AttributeModifier::IsDataValid(FDataValidationContext& Context) const
{
	EDataValidationResult Result = FArcRecipeOutputModifier::IsDataValid(Context);

	if (!BackingGameplayEffect)
	{
		Context.AddError(FText::FromString(TEXT("RecipeOutputModifier_AttributeModifier: BackingGameplayEffect is not set")));
		...
		for (const auto& [Tag, Magnitude] : BaseAttributes)
		{
			if (!EffectSetByCallerTags.Contains(Tag))
			{
				Context.AddError(FText::FromString(FString::Printf(
					TEXT("RecipeOutputModifier_AttributeModifier: Attribute tag '%s' has no matching SetByCaller modifier in %s"),
