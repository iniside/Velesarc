	...
	float AverageQuality) const
{
	...
	for (const FArcMaterialRuleEvaluation& Eval : Evaluations)
	{
		...
		for (const FInstancedStruct& ModStruct : Eval.Band->Modifiers)
		{
			...
			else if (const FArcCraftModifier_AttributeModifier* AttrMod = ModStruct.GetPtr<FArcCraftModifier_AttributeModifier>())
			{
				if (AttrMod->BackingGameplayEffect)
				{
					const float QScale = 1.0f + (BandEligibilityQuality - 1.0f) * AttrMod->QualityScalingFactor;
					ModResult.Type = EArcCraftModifierResultType::AttributeModifier;
					ModResult.AttributeModifierEffect = AttrMod->BackingGameplayEffect;
					for (const auto& [Tag, Magnitude] : AttrMod->BaseAttributes)
					{
						ModResult.AttributeModifierValues.Add(Tag, Magnitude * QScale);
					}
				}
			}
			else
