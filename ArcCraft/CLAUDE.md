...
`FArcCraftModifier` → `_Stats`, `_Abilities`, `_Effects`, `_AttributeModifier`
- Each holds a **singular** field: `BaseStat`, `AbilityToGrant`, `EffectToGrant`, `BackingGameplayEffect`+`BaseAttributes` (not arrays)
...
`FArcRecipeOutputModifier` → `_Stats`, `_Abilities`, `_Effects`, `_AttributeModifier`, `_TransferStats`, `_Random` (ChooserTable), `_RandomPool`, `_MaterialProperties`
