...
#include UE_INLINE_GENERATED_CPP_BY_NAME(ArcAlwaysLoadedEntityActor)

AArcAlwaysLoadedEntityActor::AArcAlwaysLoadedEntityActor(const FObjectInitializer& ObjectInitializer)
...
FText AArcAlwaysLoadedEntityActor::GetSMInstanceDisplayName(const FSMInstanceId& InstanceId) const
{
#if WITH_EDITOR
...
FText AArcAlwaysLoadedEntityActor::GetSMInstanceTooltip(const FSMInstanceId& InstanceId) const
{
#if WITH_EDITOR
	int32 EntryIndex = INDEX_NONE;
	int32 TransformIndex = INDEX_NONE;
...
void AArcAlwaysLoadedEntityActor::ForEachSMInstanceInSelectionGroup(const FSMInstanceId& InstanceId, TFunctionRef<bool(FSMInstanceId)> Callback)
{
#if WITH_EDITOR
	int32 EntryIndex = INDEX_NONE;
	int32 TransformIndex = INDEX_NONE;
...
bool AArcAlwaysLoadedEntityActor::CanEditSMInstance(const FSMInstanceId& InstanceId) const
{
#if WITH_EDITOR
	int32 EntryIndex = INDEX_NONE;
	int32 TransformIndex = INDEX_NONE;
...
bool AArcAlwaysLoadedEntityActor::CanMoveSMInstance(const FSMInstanceId& InstanceId, const ETypedElementWorldType WorldType) const
{
#if WITH_EDITOR
	int32 EntryIndex = INDEX_NONE;
	int32 TransformIndex = INDEX_NONE;
...
bool AArcAlwaysLoadedEntityActor::GetSMInstanceTransform(const FSMInstanceId& InstanceId, FTransform& OutInstanceTransform, bool bWorldSpace) const
{
#if WITH_EDITOR
	int32 EntryIndex = INDEX_NONE;
	int32 TransformIndex = INDEX_NONE;
...
bool AArcAlwaysLoadedEntityActor::SetSMInstanceTransform(const FSMInstanceId& InstanceId, const FTransform& InstanceTransform, bool bWorldSpace, bool bMarkRenderStateDirty, bool bTeleport)
{
#if WITH_EDITOR
	int32 EntryIndex = INDEX_NONE;
	int32 TransformIndex = INDEX_NONE;
...
void AArcAlwaysLoadedEntityActor::NotifySMInstanceMovementStarted(const FSMInstanceId& InstanceId)
{
	// No-op
}

void AArcAlwaysLoadedEntityActor::NotifySMInstanceMovementOngoing(const FSMInstanceId& InstanceId)
{
	// No-op
}

void AArcAlwaysLoadedEntityActor::NotifySMInstanceMovementEnded(const FSMInstanceId& InstanceId)
{
#if WITH_EDITOR
	Modify();
#endif
...
void AArcAlwaysLoadedEntityActor::NotifySMInstanceSelectionChanged(const FSMInstanceId& InstanceId, const bool bIsSelected)
{
#if WITH_EDITOR
	int32 EntryIndex = INDEX_NONE;
	int32 TransformIndex = INDEX_NONE;
...
bool AArcAlwaysLoadedEntityActor::DeleteSMInstances(TArrayView<const FSMInstanceId> InstanceIds)
{
#if WITH_EDITOR
	...
	// Singleton persists even when all entries are removed — no self-destruct
...
bool AArcAlwaysLoadedEntityActor::DuplicateSMInstances(TArrayView<const FSMInstanceId> InstanceIds, TArray<FSMInstanceId>& OutNewInstanceIds)
{
#if WITH_EDITOR
	if (InstanceIds.Num() == 0)
	{
...
TSubclassOf<USMInstanceProxyEditingObject> AArcAlwaysLoadedEntityActor::GetSMInstanceEditingProxyClass() const
{
#if WITH_EDITOR
...
FText AArcAlwaysLoadedEntityActor::GetSkMInstanceDisplayName(const FSkMInstanceId& InstanceId) const
{
#if WITH_EDITOR
	int32 EntryIndex = INDEX_NONE;
	int32 TransformIndex = INDEX_NONE;
...
FText AArcAlwaysLoadedEntityActor::GetSkMInstanceTooltip(const FSkMInstanceId& InstanceId) const
{
#if WITH_EDITOR
	int32 EntryIndex = INDEX_NONE;
	int32 TransformIndex = INDEX_NONE;
...
void AArcAlwaysLoadedEntityActor::ForEachSkMInstanceInSelectionGroup(const FSkMInstanceId& InstanceId, TFunctionRef<bool(FSkMInstanceId)> Callback)
{
#if WITH_EDITOR
	int32 EntryIndex = INDEX_NONE;
	int32 TransformIndex = INDEX_NONE;
...
bool AArcAlwaysLoadedEntityActor::CanEditSkMInstance(const FSkMInstanceId& InstanceId) const
{
#if WITH_EDITOR
	int32 EntryIndex = INDEX_NONE;
	int32 TransformIndex = INDEX_NONE;
...
bool AArcAlwaysLoadedEntityActor::CanMoveSkMInstance(const FSkMInstanceId& InstanceId, const ETypedElementWorldType WorldType) const
{
#if WITH_EDITOR
	int32 EntryIndex = INDEX_NONE;
	int32 TransformIndex = INDEX_NONE;
...
bool AArcAlwaysLoadedEntityActor::GetSkMInstanceTransform(const FSkMInstanceId& InstanceId, FTransform& OutInstanceTransform, bool bWorldSpace) const
{
#if WITH_EDITOR
	int32 EntryIndex = INDEX_NONE;
	int32 TransformIndex = INDEX_NONE;
...
bool AArcAlwaysLoadedEntityActor::SetSkMInstanceTransform(const FSkMInstanceId& InstanceId, const FTransform& InstanceTransform, bool bWorldSpace, bool bMarkRenderStateDirty, bool bTeleport)
{
#if WITH_EDITOR
	int32 EntryIndex = INDEX_NONE;
	int32 TransformIndex = INDEX_NONE;
...
void AArcAlwaysLoadedEntityActor::NotifySkMInstanceMovementStarted(const FSkMInstanceId& InstanceId)
{
	// No-op
}

void AArcAlwaysLoadedEntityActor::NotifySkMInstanceMovementOngoing(const FSkMInstanceId& InstanceId)
{
	// No-op
}

void AArcAlwaysLoadedEntityActor::NotifySkMInstanceMovementEnded(const FSkMInstanceId& InstanceId)
{
#if WITH_EDITOR
	Modify();
#endif
...
void AArcAlwaysLoadedEntityActor::NotifySkMInstanceSelectionChanged(const FSkMInstanceId& InstanceId, const bool bIsSelected)
{
	// No-op — skinned mesh instances do not support per-instance selection highlighting yet
}

bool AArcAlwaysLoadedEntityActor::DeleteSkMInstances(TArrayView<const FSkMInstanceId> InstanceIds)
{
#if WITH_EDITOR
	if (InstanceIds.Num() == 0)
	{
...
bool AArcAlwaysLoadedEntityActor::DuplicateSkMInstances(TArrayView<const FSkMInstanceId> InstanceIds, TArray<FSkMInstanceId>& OutNewInstanceIds)
{
#if WITH_EDITOR
	if (InstanceIds.Num() == 0)
	{
...
ISkMInstanceManager* AArcAlwaysLoadedEntityActor::GetSkMInstanceManager(const FSkMInstanceId& InstanceId)
{
...
ISMInstanceManager* AArcAlwaysLoadedEntityActor::GetSMInstanceManager(const FSMInstanceId& InstanceId)
{
#if WITH_EDITORONLY_DATA
...
void AArcAlwaysLoadedEntityActor::BeginPlay()
{
	Super::BeginPlay();
	SpawnEntities();
}

void AArcAlwaysLoadedEntityActor::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	DespawnEntities();
	Super::EndPlay(EndPlayReason);
}

void AArcAlwaysLoadedEntityActor::SpawnEntities()
{
	...
	for (const FArcPlacedEntityEntry& Entry : Entries)
	{
		...
		{
			UE_LOG(LogMass, Warning, TEXT("AArcAlwaysLoadedEntityActor::SpawnEntities: Invalid archetype for config '%s'"),
...
void AArcAlwaysLoadedEntityActor::DespawnEntities()
{
	UWorld* World = GetWorld();
	if (World == nullptr)
	{
...
void AArcAlwaysLoadedEntityActor::BeginDestroy()
{
#if WITH_EDITOR
...
void AArcAlwaysLoadedEntityActor::PostRegisterAllComponents()
{
	...
	if (World != nullptr && !World->IsGameWorld())
	{
		...
		FEditorDelegates::PreBeginPIE.Remove(PreBeginPIEHandle);
		FEditorDelegates::EndPIE.Remove(EndPIEHandle);
		PreBeginPIEHandle = FEditorDelegates::PreBeginPIE.AddUObject(this, &AArcAlwaysLoadedEntityActor::OnPreBeginPIE);
		EndPIEHandle = FEditorDelegates::EndPIE.AddUObject(this, &AArcAlwaysLoadedEntityActor::OnEndPIE);
	}
}

void AArcAlwaysLoadedEntityActor::OnPreBeginPIE(const bool bIsSimulating)
{
	DestroyEditorPreviewISMCs();
}

void AArcAlwaysLoadedEntityActor::OnEndPIE(const bool bIsSimulating)
{
	RebuildEditorPreviewISMCs();
}

void AArcAlwaysLoadedEntityActor::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
...
void AArcAlwaysLoadedEntityActor::PostLoad()
{
	Super::PostLoad();
...
void AArcAlwaysLoadedEntityActor::AutoAssignLinkGuidIfNeeded(FArcPlacedEntityEntry& Entry, int32 InstanceIndex)
{
	if (Entry.EntityConfig == nullptr)
	{
		return;
...
void AArcAlwaysLoadedEntityActor::AddInstance(UMassEntityConfigAsset* ConfigAsset, const FTransform& Transform, int32& OutEntryIndex, int32& OutInstanceIndex)
{
...
UInstancedStaticMeshComponent* AArcAlwaysLoadedEntityActor::GetEditorPreviewISMC(int32 EntryIndex) const
{
...
void AArcAlwaysLoadedEntityActor::RebuildEditorPreviewISMCs()
{
...
void AArcAlwaysLoadedEntityActor::DestroyEditorPreviewISMCs()
{
	if (CollisionDrawComponent != nullptr)
	{
...
void AArcAlwaysLoadedEntityActor::RebuildGuidIndex()
{
...
bool AArcAlwaysLoadedEntityActor::FindInstanceByGuid(const FGuid& Guid, int32& OutEntryIndex, int32& OutTransformIndex) const
{
...
bool AArcAlwaysLoadedEntityActor::ResolveInstanceId(const FSMInstanceId& InstanceId, int32& OutEntryIndex, int32& OutTransformIndex) const
{
	UInstancedStaticMeshComponent* Component = InstanceId.ISMComponent;
	if (Component == nullptr)
	{
...
bool AArcAlwaysLoadedEntityActor::ResolveSkMInstanceId(const FSkMInstanceId& InstanceId, int32& OutEntryIndex, int32& OutTransformIndex) const
{
	UInstancedSkinnedMeshComponent* Component = InstanceId.SkMComponent;
	if (Component == nullptr)
	{
...
UInstancedSkinnedMeshComponent* AArcAlwaysLoadedEntityActor::GetEditorPreviewSkMC(int32 EntryIndex) const
{
