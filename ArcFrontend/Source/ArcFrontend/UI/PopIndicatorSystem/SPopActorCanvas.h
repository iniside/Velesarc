...
class UArcPopIndicatorManagerComponent;

class SPopActorCanvas : public SPanel, public FAsyncMixin
{
public:
	float Speed = 1;
	...
	public: 
		...
		FSlot(const TSharedRef<FPopIndicatorDescriptor>& CanvasEntry)
			: TSlotBase<FSlot>()
	...
	private:
		...
		TSharedRef<FPopIndicatorDescriptor> ActorEntry;
	...
	SLATE_BEGIN_ARGS(SPopActorCanvas) {
		_Visibility = EVisibility::HitTestInvisible;
	}

		/** Indicates that we have a slot that this widget supports */
		SLATE_SLOT_ARGUMENT(SPopActorCanvas::FSlot, Slots)
...
private:
	void OnIndicatorAdded(const TSharedRef<FPopIndicatorDescriptor>& CanvasEntry, TFunctionRef<void(UUserWidget*)> WidgetInit);

	void AddIndicatorForEntry(const TSharedRef<FPopIndicatorDescriptor>& CanvasEntry, TFunctionRef<void(UUserWidget*)> WidgetInit);
	void RemoveIndicatorForEntry(const TSharedPtr<FPopIndicatorDescriptor>& CanvasEntry);

	using FScopedWidgetSlotArguments = TPanelChildren<FSlot>::FScopedWidgetSlotArguments;
	FScopedWidgetSlotArguments AddActorSlot(const TSharedRef<FPopIndicatorDescriptor>& CanvasEntry);
	...
	void GetOffsetAndSize(const TSharedRef<FPopIndicatorDescriptor>& Entry,
		FVector2D& OutSize, 
		FVector2D& OutOffset,
...
private:
	...
	TWeakObjectPtr<UArcPopIndicatorManagerComponent> IndicatorComponentPtr;
