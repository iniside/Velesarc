...
ULyraSettingsLocal::ULyraSettingsLocal()
{
	...
	if (!HasAnyFlags(RF_ClassDefaultObject) && FSlateApplication::IsInitialized())
	{
		...
		if(UArcLocalPlayerBase::GetLocalSettingsDelegate.IsBound())
		{
			UArcLocalPlayerBase::GetLocalSettingsDelegate.Unbind();

			UArcLocalPlayerBase::GetLocalSettingsDelegate.BindStatic(&ULyraSettingsLocal::GetUserSettings);
		}
	}
	SetToDefaults();
}
...
void ULyraSettingsLocal::LoadSettings(bool bForceReload)
{
	...
	HasPlatformTrait::FillScalabilitySettingsFromDeviceProfile(DeviceDefaultScalabilitySettings);
...
UGameUserSettings* ULyraSettingsLocal::GetUserSettings()
{
...
float ULyraSettingsLocal::GetEffectiveFrameRateLimit()
{
	...
	{
		EffectiveFrameRateLimit = Arcx::Settings::CombineFrameRateLimits(EffectiveFrameRateLimit, FrameRateLimit_InMenu);
	}
	...
	if (PlatformSettings->FramePacingMode == ELyraFramePacingMode::DesktopStyle)
	{
		...
		{
			EffectiveFrameRateLimit = Arcx::Settings::CombineFrameRateLimits(EffectiveFrameRateLimit, FrameRateLimit_OnBattery);
		}

		if (FSlateApplication::IsInitialized() && !FSlateApplication::Get().IsActive())
		{
			EffectiveFrameRateLimit = Arcx::Settings::CombineFrameRateLimits(EffectiveFrameRateLimit, FrameRateLimit_WhenBackgrounded);
		}
	}
...
{
	return HasPlatformTrait::GetHighestLevelOfAnyScalabilityChannel(ScalabilityQuality);
}
...
{
	return HasPlatformTrait::GetFirstFrameRateWithQualityLimit();
}

int32 ULyraSettingsLocal::GetLowestQualityWithFrameRateLimit() const
{
	return HasPlatformTrait::GetLowestQualityWithFrameRateLimit();
}
...
int32 ULyraSettingsLocal::GetMaxSupportedOverallQualityLevel() const
{
	...
	{
		return HasPlatformTrait::GetHighestLevelOfAnyScalabilityChannel(DeviceDefaultScalabilitySettings.Qualities);
	}
	else
	{
		return 3;
...
void ULyraSettingsLocal::ClampMobileFPSQualityLevels(bool bWriteBack)
{
	...
	if (PlatformSettings->FramePacingMode == ELyraFramePacingMode::MobileStyle)
	{
		const int32 QualityLimit = HasPlatformTrait::GetApplicableOverallQualityLimit(DesiredMobileFrameRateLimit);
...
void ULyraSettingsLocal::ClampMobileResolutionQuality(int32 TargetFPS)
{
	...
	float MaxMobileResQuality = HasPlatformTrait::GetApplicableResolutionQualityLimit(TargetFPS);
...
void ULyraSettingsLocal::RemapMobileResolutionQuality(int32 FromFPS, int32 ToFPS)
{
	...
	float MaxScaleValue = 0.0f;
	GetResolutionScaleInformationEx(CurrentScaleNormalized, CurrentScaleValue, MinScaleValue, MaxScaleValue);
	float FromMaxMobileResQuality = HasPlatformTrait::GetApplicableResolutionQualityLimit(FromFPS);
	float ToMaxMobileResQuality = HasPlatformTrait::GetApplicableResolutionQualityLimit(ToFPS);
...
bool ULyraSettingsLocal::CanModifyHeadphoneModeEnabled() const
{
	...
	const bool bBinauralSettingControlledByOS = HasPlatformTrait::HasPlatformTrait(TAG_Platform_Trait_BinauralSettingControlledByOS);
...
void ULyraSettingsLocal::SetOverallScalabilityLevel(int32 Value)
{
	...
	if (PlatformSettings->FramePacingMode == ELyraFramePacingMode::MobileStyle)
	{
		...
		const int32 ConstrainedFrameRateLimit = HasPlatformTrait::ConstrainFrameRateToBeCompatibleWithOverallQuality(DesiredMobileFrameRateLimit, Value);
