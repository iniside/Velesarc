...
void SArcPropertyBinding::Construct(const FArguments& InArgs, UBlueprint* InBlueprint, const TArray<FBindingContextStruct>& InBindingContextStructs)
{
	...
	[
		...
		[
			...
			.ToolTipText(this, &SArcPropertyBinding::GetCurrentBindingToolTipText)
			.OnGetMenuContent(this, &SArcPropertyBinding::OnGenerateDelegateMenu)
			.ContentPadding(1)
			.ButtonContent()
			[
				SNew(SHorizontalBox)
				...
				[
					...
					[
						SNew(SImage)
						.Image(this, &SArcPropertyBinding::GetCurrentBindingImage)
						.ColorAndOpacity(this, &SArcPropertyBinding::GetCurrentBindingColor)
					]
				]
				+ SHorizontalBox::Slot()
				.AutoWidth()
				...
				[
					SNew(STextBlock)
					.Text(this, &SArcPropertyBinding::GetCurrentBindingText)
		...
		[
			...
			.Visibility(this, &SArcPropertyBinding::GetGotoBindingVisibility)
			.OnClicked(this, &SArcPropertyBinding::HandleGotoBindingClicked)
			.VAlign(VAlign_Center)
...
bool SArcPropertyBinding::IsClassDenied(UClass* InClass) const
{
...
bool SArcPropertyBinding::IsFieldFromDeniedClass(FFieldVariant Field) const
{
	return IsClassDenied(Field.GetOwnerClass());
}

template <typename Predicate>
void SArcPropertyBinding::ForEachBindableFunction(UClass* FromClass, Predicate Pred) const
{
...
template <typename Predicate>
void SArcPropertyBinding::ForEachBindableProperty(UStruct* InStruct, const TArray<TSharedPtr<FBindingChainElement>>& BindingChain, Predicate Pred) const
{
...
bool SArcPropertyBinding::HasBindableProperties(UStruct* InStruct, TArray<TSharedPtr<FBindingChainElement>>& BindingChain) const
{
	TSet<UStruct*> VisitedStructs;
	return HasBindablePropertiesRecursive(InStruct, VisitedStructs, BindingChain);
}

bool SArcPropertyBinding::HasBindablePropertiesRecursive(UStruct* InStruct, TSet<UStruct*>& VisitedStructs, TArray<TSharedPtr<FBindingChainElement>>& BindingChain) const
{
	...
	if(UClass* Class = Cast<UClass>(InStruct))
	{
		ForEachBindableFunction(Class, [this, &BindableCount, &VisitedStructs, &BindingChain](const FFunctionInfo& Info)
		{
			...
			if(Args.bGeneratePureBindings || Args.bGenerateAllowNonPureBindings)
			{
...
TSharedRef<SWidget> SArcPropertyBinding::OnGenerateDelegateMenu()
{
	...
	if ( CanRemoveBinding() )
	{
		MenuBuilder.AddMenuEntry(
			...
			FUIAction(FExecuteAction::CreateSP(this, &SArcPropertyBinding::HandleRemoveBinding))
			);
	}
	...
	if(Args.bAllowNewBindings && Args.BindableSignature != nullptr)
	{
		MenuBuilder.AddMenuEntry(
			...
			FUIAction(FExecuteAction::CreateSP(this, &SArcPropertyBinding::HandleCreateAndAddBinding))
			);
	}
	...
	if (BindingContextStructs.Num() > 0)
	{
		...
		for (int32 i = 0; i < BindingContextStructs.Num(); i++)
		{
			...
			if (bDeprecatedCanBindToContextStruct || (Args.OnCanBindToContextStructWithIndex.IsBound() && Args.OnCanBindToContextStructWithIndex.Execute(ContextStruct.Struct, i)))
			{
				...
				MenuBuilder.AddMenuEntry(
					FUIAction(FExecuteAction::CreateSP(this, &SArcPropertyBinding::HandleAddBinding, BindingChain)),
			...
			if (HasBindableProperties(ContextStruct.Struct, BindingChain))
			{
				...
				MenuBuilder.AddSubMenu(
					MakeContextStructWidget(ContextStruct),
					FNewMenuDelegate::CreateSP(this, &SArcPropertyBinding::FillPropertyMenu, ContextStruct.Struct, BindingChain));
			}
		}
...
void SArcPropertyBinding::FillPropertyMenu(FMenuBuilder& MenuBuilder, UStruct* InOwnerStruct, TArray<TSharedPtr<FBindingChainElement>> InBindingChain)
{
		...
		return SNew(SHorizontalBox)
			...
			[
				...
				[
					...
					.OnValueCommitted(this, &SArcPropertyBinding::HandleSetBindingArrayIndex, InProperty, InNewBindingChain)
				]
			];
	};
	...
	{
		MenuBuilder.AddMenuEntry(
			FUIAction(FExecuteAction::CreateSP(this, &SArcPropertyBinding::HandleAddBinding, InNewBindingChain)),
			SNew(SHorizontalBox)
			+SHorizontalBox::Slot()
			.FillWidth(1.0f)
			[
		...
		if ( UClass* OwnerClass = Cast<UClass>(InOwnerStruct) )
		{
			...
			MenuBuilder.BeginSection("Functions", LOCTEXT("Functions", "Functions"));
			{
				ForEachBindableFunction(OwnerClass, [this, &InBindingChain, &MenuBuilder, &MakeFunctionWidget] (const FFunctionInfo& Info) 
				{
					...
					{
						MenuBuilder.AddMenuEntry(
							FUIAction(FExecuteAction::CreateSP(this, &SArcPropertyBinding::HandleAddBinding, NewBindingChain)),
					...
					if(Args.bGeneratePureBindings  || Args.bGenerateAllowNonPureBindings)
					{
						if(FObjectPropertyBase* ObjectPropertyBase = CastField<FObjectPropertyBase>(ReturnProperty))
						{
							if (HasBindableProperties(ObjectPropertyBase->PropertyClass, NewBindingChain))
							{
								MenuBuilder.AddSubMenu(
									MakeFunctionWidget(Info),
									FNewMenuDelegate::CreateSP(this, &SArcPropertyBinding::FillPropertyMenu, static_cast<UStruct*>(ObjectPropertyBase->PropertyClass), NewBindingChain));
							}
						}
						else if(FStructProperty* StructProperty = CastField<FStructProperty>(ReturnProperty))
						{
							if (HasBindableProperties(StructProperty->Struct, NewBindingChain))
							{
								MenuBuilder.AddSubMenu(
									MakeFunctionWidget(Info),
									FNewMenuDelegate::CreateSP(this, &SArcPropertyBinding::FillPropertyMenu, static_cast<UStruct*>(StructProperty->Struct), NewBindingChain));
							}
						}
					}
				});
	...
	if ( (Args.bGeneratePureBindings && Args.bAllowPropertyBindings) || Args.bGenerateAllowNonPureBindings )
	{
						...
						if (HasBindableProperties(Struct, NewBindingChain))
						{
							if (Args.bAllowArrayElementBindings && ArrayProperty != nullptr)
							{
								...
								MenuBuilder.AddSubMenu(
									...
									FNewMenuDelegate::CreateSP(this, &SArcPropertyBinding::FillPropertyMenu, Struct, NewArrayElementBindingChain)
									);
							}
							else if (Args.bAllowStructMemberBindings)
							{
								MenuBuilder.AddSubMenu(
									...
									FNewMenuDelegate::CreateSP(this, &SArcPropertyBinding::FillPropertyMenu, Struct, NewBindingChain));
							}
						}
					}
				});
...
const FSlateBrush* SArcPropertyBinding::GetCurrentBindingImage() const
{
...
FText SArcPropertyBinding::GetCurrentBindingText() const
{
...
FText SArcPropertyBinding::GetCurrentBindingToolTipText() const
{
...
FSlateColor SArcPropertyBinding::GetCurrentBindingColor() const
{
...
bool SArcPropertyBinding::CanRemoveBinding()
{
...
void SArcPropertyBinding::HandleRemoveBinding()
{
...
void SArcPropertyBinding::HandleAddBinding(TArray<TSharedPtr<FBindingChainElement>> InBindingChain)
{
...
void SArcPropertyBinding::HandleSetBindingArrayIndex(int32 InArrayIndex, ETextCommit::Type InCommitType, FProperty* InProperty, TArray<TSharedPtr<FBindingChainElement>> InBindingChain)
{
...
void SArcPropertyBinding::HandleCreateAndAddBinding()
{
	if (!Blueprint)
	{
		return;
...
UStruct* SArcPropertyBinding::ResolveIndirection(const TArray<TSharedPtr<FBindingChainElement>>& InBindingChain) const
{
...
EVisibility SArcPropertyBinding::GetGotoBindingVisibility() const
{
...
FReply SArcPropertyBinding::HandleGotoBindingClicked()
{
...
bool SArcPropertyBinding::CanAcceptPropertyOrChildren(FProperty* InProperty, TConstArrayView<TSharedPtr<FBindingChainElement>> InBindingChain) const
{
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
...
bool SArcPropertyBinding::CanBindProperty(FProperty* InProperty, TConstArrayView<TSharedPtr<FBindingChainElement>> InBindingChain) const
{
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
...
FReply SArcPropertyBinding::OnDrop(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent)
{
